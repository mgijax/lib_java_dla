...
public class DBGSSGeneTrapInterpreter extends GBFormatInterpreter {
	...
	public DBGSSGeneTrapInterpreter(GBOrganismChecker oc)
		...
		//System.out.println("Mutation in Interpreter: " + molecularMutation);
...
/*
	...
	private DerivationRawAttributes interpretDerivation(
		...
		//System.out.println(sequenceRaw.getLibrary());
	...
	private String interpretSeqGeneTrap(SequenceInput seqInput,
		...
		//System.out.println("DBGSSGeneTrapInterpreter.interpretSeqGeneTrap seqTagMethod" + seqGTRaw.getSeqTagMethod());
		return cellLineID;
	}
	...
	/**
	 * parse sequence tag method from the SequenceInput COMMENT or /note= (TIGM)
	 * @param seqInput - Raw Sequence Data from a GenBank Gene Trap sequence rcd
	...
	genomic DNA"
	 */
	private String getSeqTagMethod(SequenceInput seqInput) {

		// check the comment section first
		...
		if (comment != null) {
			...
			// check the note attribute if we didn't find it in comment (TIGM)
			if (method == null) {
				//System.out.println(seqInput.getPrimaryAcc().getAccID());
				String note = seqInput.getSeq().getNote();
				if (note != null) {
		...
		// sequence tag method not found, report it
		if (method == null) {
